...
time seq 1 10 | xargs -n1 -P3 bash -c 'i=$0; cmd="tiff_conv"; url="http://localhost:8080/cmd?run=${i}"; curl -d "cmd=$cmd&i=$i" -X POST $url'

time seq 1 10 | xargs -n1 -P3 bash -c 'i=$0; cmd="tiff_resize"; url="http://localhost:8080/cmd?run=${i}"; curl -d "cmd=$cmd&i=$i" -X POST $url'
